...
# for bulk openai message, no stream
def chat_openai_tailor_essay(essay="Tell me to ask you a prompt", sample="", chat_history=[]):
    if not essay or not sample:
        return
    ...
    if sample:
        prompt = "Essay generated by a LLM: " + essay[:2000] + "\n Sample essay from the author: " + sample[:2000]
    ...
    # messages.append({"role": "user", "content": prompt})
    messages.append({"role": "user", "content": "I want you to look at and analyze some sample text, and then copy the writing style as closely as possible according to my requests."})
    messages.append({"role": "assistant", "content": "Sure, I can try to emulate different writing styles based on your requests. Please provide me with the sample text and your desired style."})
    messages.append({"role": "user", "content": "Here is the sample: " + sample[:MAX_HALF_CHAT_SIZE]})
    messages.append({"role": "assistant", "content": "Sure, which writing style would you like me to emulate for you?"})
    messages.append({"role": "user", "content": "Please edit this essay using the sample style as closely as possible so that the new essay style is almost indistinguishable from the sample style, and explain what edits you made at the end: " + essay[:MAX_HALF_CHAT_SIZE]})
    print("messages", messages)
...
# chat_openai_tailor_essay("temp", "temp")
